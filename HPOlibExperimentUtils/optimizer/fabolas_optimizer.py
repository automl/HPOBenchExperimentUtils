    ...
    def __init__(self, benchmark: Union[Bookkeeper, AbstractBenchmark, AbstractBenchmarkClient],
        ...
        if isinstance(self.main_fidelity, cs.UniformIntegerHyperparameter):
            ...
            _log.debug("Treating float fidelity parameter %s as the main fidelity used for dataset subsampling." %
                       self.main_fidelity.name)
            assert hasattr(benchmark, 'X_train'), "The benchmark object is expected to have an attribute 'X_train' " \
                                                  "in order to be compatible with FABOLAS."
            assert hasattr(benchmark, 'y_train'), "The benchmark object is expected to have an attribute 'y_train' " \
                                                  "in order to be compatible with FABOLAS."
            assert 0.0 <= self.main_fidelity.lower and self.main_fidelity.upper <= 1.0
            self.s_min = max(self.main_fidelity.lower * benchmark.y_train.shape[0], 1)
            self.s_max = self.main_fidelity.upper * benchmark.y_train.shape[0]
        else:
