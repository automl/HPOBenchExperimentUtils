...
def generate_trajectory(history: Namespace, save_file: Path, is_cp=False):
    """
    ...
    to be saved. The is_cp flag indicates that a Cartesian Product space was used, thus affecting the output format.
    """
    trajectories = []
    incumbent = {
        "cpu_time": float(0),
    ...
    for qinfo in history.query_qinfos:
        ...
        if -qinfo.val < incumbent["cost"]:
            incumbent = {
                ...
                "cost": -qinfo.val,
                "incumbent": list(qinfo.point[0]) if is_cp else list(qinfo.point),
