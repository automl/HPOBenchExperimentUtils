...
if __name__ == "__main__":
    ...
    parser.add_argument('--what', choices=["all", "table", "over_time", "other", "ecdf",
    ...
    if args.what in ("all", "ecdf"):
        if args.unvalidated is False:
            _log.critical("ECDF will be plotted on unvalidated data")
        plot_ecdf(criterion=args.agg, **vars(args))

    if args.what in ("all", "correlation"):
        if args.unvalidated is False:
            _log.critical("Correlations will be plotted on unvalidated data")
        plot_correlation(criterion=args.agg, **vars(args))
